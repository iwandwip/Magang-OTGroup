...
void showHelp() {
  ...
  Serial.print(F("SET EXTEND_SPEED=value        - Set extend motion speed (current: "));
  Serial.print(extendMaxSpeed);
  Serial.println(F(")"));
  Serial.print(F("SET EXTEND_ACCEL=value        - Set extend acceleration (current: "));
  Serial.print(extendAcceleration);
  Serial.println(F(")"));
  Serial.print(F("SET EXTEND_DELAY=value        - Set extend delay (current: "));
  Serial.print(extendDelayBefore);
  Serial.println(F("ms)"));
  Serial.print(F("SET RETRACT_SPEED=value       - Set retract motion speed (current: "));
  Serial.print(retractMaxSpeed);
  Serial.println(F(")"));
  Serial.print(F("SET RETRACT_ACCEL=value       - Set retract acceleration (current: "));
  Serial.print(retractAcceleration);
  Serial.println(F(")"));
  Serial.print(F("SET RETRACT_DELAY_BEFORE=val  - Set retract delay before (current: "));
  Serial.print(retractDelayBefore);
  Serial.println(F("ms)"));
  Serial.print(F("SET RETRACT_DELAY_AFTER=val   - Set retract delay after (current: "));
  Serial.print(retractDelayAfter);
  Serial.println(F("ms)"));
  Serial.print(F("SET RETRACT_ADJUSTMENT=val    - Set retract step adjustment (current: "));
  Serial.print(retractStepAdjustment);
  Serial.println(F(")"));
  Serial.println(F("--- Motion Smoothness Commands ---"));
  Serial.print(F("SET JERK_STEPS=val            - Set S-curve jerk reduction steps (current: "));
  Serial.print(jerkReductionSteps);
  Serial.println(F(")"));
  Serial.print(F("SET ENABLE_RAMP=val           - Set enable ramp delay (current: "));
  Serial.print(enableRampDelay);
  Serial.println(F("ms)"));
  Serial.print(F("SET DISABLE_RAMP=val          - Set disable ramp delay (current: "));
  Serial.print(disableRampDelay);
  Serial.println(F("ms)"));
  Serial.print(F("SET SPEED_RAMP=val            - Set speed ramp factor (current: "));
  Serial.print(speedRampFactor);
  Serial.println(F(")"));
  Serial.print(F("SET SETTLE_DELAY=val          - Set motion settle delay (current: "));
  Serial.print(motionSettleDelay);
  Serial.println(F("ms)"));
  Serial.println(F("--- LED Control Commands ---"));
  Serial.print(F("SET LED_IDLE=val              - Set idle LED period (current: "));
  Serial.print(ledIdlePeriod / 1000);
  Serial.println(F("ms)"));
  Serial.print(F("SET LED_EXTEND=val            - Set extend LED period (current: "));
  Serial.print(ledExtendPeriod / 1000);
  Serial.println(F("ms)"));
  Serial.print(F("SET LED_RETRACT=val           - Set retract LED period (current: "));
  Serial.print(ledRetractPeriod / 1000);
  Serial.println(F("ms)"));
  Serial.print(F("SET LED_ERROR=val             - Set error LED period (current: "));
  Serial.print(ledErrorPeriod / 1000);
  Serial.println(F("ms)"));
  Serial.print(F("SET LED_DEBUG=val             - Set debug LED period (current: "));
  Serial.print(ledDebugPeriod / 1000);
  Serial.println(F("ms)"));
